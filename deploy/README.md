...
**Panel-only tree:** install/update persist `.scrapeboard-role=panel` (or `SCRAPEBOARD_ROLE=panel`) and use git sparse-checkout to exclude `worker/` (and remove it if present). Scrape agents are installed from [`../worker/`](../worker/) on separate hosts — not on the panel VPS. Worker hosts use `.scrapeboard-role=worker` and exclude `panel/` + `deploy/`. Full local clones for development are unchanged until you opt into sparse-checkout.
